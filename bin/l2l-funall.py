...
def main():
    ...
    traj, _ = experiment.prepare_experiment(
        name=name, log_stdout=True, jube_parameter=jube_params
    )
    n_iterations = 100
    seed = 1
    ...
    optimizers = [
        ...
        (
            GradientDescentOptimizer,
            lambda: AdamParameters(
                learning_rate=0.01,
                exploration_step_size=0.01,
                n_random_steps=5,
                first_order_decay=0.8,
                second_order_decay=0.8,
                n_iteration=n_iterations,
                stop_criterion=np.Inf,
                seed=seed,
            ),
    ...
    for function_id, (optimizer_class, optimizer_parameters_fn) in itertools.product(
        function_ids, optimizers
    ):
        ...
        # Grid search optimizer input depends on optimizee!
            ...
            optimizer_parameters = GridSearchParameters(
                param_grid={"coords": (optimizee.bound[0], optimizee.bound[1], 30)}
            )
