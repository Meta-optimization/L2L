...
class GridSearchOptimizer(Optimizer):
    ...
    def __init__(
        ...
        self.param_list = {}
        _, optimizee_individual_param_spec = dict_to_list(
            sample_individual, get_dict_spec=True
        )
        ...
        assert set(sample_individual.keys()) == set(
            optimizee_param_grid.keys()
        ...
        for param_name, param_type, param_length in optimizee_individual_param_spec:
            ...
            if param_type == DictEntryType.Scalar:
                self.param_list[param_name] = np.linspace(
                    param_lower_bound, param_upper_bound, param_n_steps + 1
                )
            elif param_type == DictEntryType.Sequence:
                curr_param_list = np.linspace(
                    param_lower_bound, param_upper_bound, param_n_steps + 1
                )
                curr_param_list = np.meshgrid(
                    *([curr_param_list] * param_length), indexing="ij"
                )
        ...
        for param_name, param_grid_spec in optimizee_param_grid.items():
            traj.grid_spec.f_add_parameter(
                param_name + ".lower_bound", param_grid_spec[0]
            )
            traj.grid_spec.f_add_parameter(
                param_name + ".uper_bound", param_grid_spec[1]
            )
        traj.f_add_parameter(
            "n_iteration", 1, comment="Grid search does only 1 iteration"
        )
        #: The current generation number
        self.g = 0
        # Expanding the trajectory
        grouped_params_dict = {
            "individual." + key: value for key, value in self.param_list.items()
        }
        final_params_dict = {"generation": [self.g], "ind_idx": range(self.size)}
    ...
    def post_process(self, traj, fitnesses_results):
        ...
        optimizee_fitness_weights = np.reshape(
            np.array(self.optimizee_fitness_weights), (-1, 1)
        )

        weighted_fitness_array = np.multiply(
            fitness_array, optimizee_fitness_weights
        ).ravel()
        ...
        for run_idx, run_fitness, run_weighted_fitness in zip(
            run_idx_array, fitness_array, weighted_fitness_array
        ):
            traj.v_idx = run_idx
        ...
        logger.info(
            "  with weighted fitness: %s",
            weighted_fitness_array[max_fitness_indiv_index],
        )
