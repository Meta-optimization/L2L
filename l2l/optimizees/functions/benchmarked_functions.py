from l2l.optimizees.functions.function_generator import (
    ...
    ShekelParameters,
    EasomParameters,
    LangermannParameters,
    RastriginParameters,
    ChasmParameters,
    RosenbrockParameters,
    AckleyParameters,
...
class BenchmarkedFunctions:
    ...
    def __init__(self):
        self.function_name_map = [
            ("Rastrigin2d", self._create_rastrigin2d),
        ...
        self.function_name_index_map = OrderedDict(
            [(name, index) for index, (name, _) in enumerate(self.function_name_map)]
        )

    def get_function_by_index(self, id_, noise=False, mu=0.0, sigma=0.01):
        """
        ...
        return (return_function_name, return_function), self.get_params(
            return_function, id_
        )

    def get_function_by_name(self, name, noise=False, mu=0.0, sigma=0.01):
        """
        Get the benchmarked function with given id
    ...
    def _create_rastrigin2d(self, noise, mu, sigma):
        return FunctionGenerator(
            [RastriginParameters()], dims=2, noise=noise, mu=mu, sigma=sigma
        )

    def _create_rastrigin10d(self, noise, mu, sigma):
        return FunctionGenerator(
            [RastriginParameters()], dims=10, noise=noise, mu=mu, sigma=sigma
        )

    def _create_rosenbrock2d(self, noise, mu, sigma):
        return FunctionGenerator(
            [RosenbrockParameters()], dims=2, noise=noise, mu=mu, sigma=sigma
        )

    def _create_rosenbrock10d(self, noise, mu, sigma):
        return FunctionGenerator(
            [RosenbrockParameters()], dims=10, noise=noise, mu=mu, sigma=sigma
        )

    def _create_ackley2d(self, noise, mu, sigma):
        return FunctionGenerator(
            [AckleyParameters()], dims=2, noise=noise, mu=mu, sigma=sigma
        )

    def _create_ackley10d(self, noise, mu, sigma):
        return FunctionGenerator(
            [AckleyParameters()], dims=10, noise=noise, mu=mu, sigma=sigma
        )

    def _create_chasm(self, noise, mu, sigma):
        return FunctionGenerator(
            [ChasmParameters()], dims=2, noise=noise, mu=mu, sigma=sigma
        )

    def _create_gauss2d(self, noise, mu, sigma):
        return FunctionGenerator(
            [GaussianParameters(sigma=[[1.5, 0.1], [0.1, 0.3]], mean=[-1.0, -1.0])],
            dims=2,
            noise=noise,
            mu=mu,
            sigma=sigma,
    ...
    def _create_michalewicz2d(self, noise, mu, sigma):
        return FunctionGenerator(
            [MichalewiczParameters(m="default")],
            dims=2,
            noise=noise,
            mu=mu,
            sigma=sigma,
    ...
    def _create_permutation2d(self, noise, mu, sigma):
        return FunctionGenerator(
            [PermutationParameters(beta=0.005)], dims=2, noise=noise, mu=mu, sigma=sigma
        )

    def _create_easom2d(self, noise, mu, sigma):
        return FunctionGenerator(
            [EasomParameters()], dims=2, noise=noise, mu=mu, sigma=sigma
        )

    def _create_easom10d(self, noise, mu, sigma):
        return FunctionGenerator(
            [EasomParameters()], dims=10, noise=noise, mu=mu, sigma=sigma
        )
    ...
    def _create_3gaussians2d(self, noise, mu, sigma):
        fg_params = [
            GaussianParameters(sigma=[[1.5, 0.1], [0.1, 0.3]], mean=[-1.0, -1.0]),
            GaussianParameters(sigma=[[0.25, 0.3], [0.3, 1.0]], mean=[1.0, 1.0]),
            GaussianParameters(sigma=[[0.5, 0.25], [0.25, 1.3]], mean=[2.0, -2.0]),
        ]
