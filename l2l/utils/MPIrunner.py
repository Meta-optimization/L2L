...
class MPIRunner():
    ...
    def simulate_generation(self, gen, n_inds):
        ...
        sub_comms = []
        logger.info("Launching individuals")#.format(idx))
        sub_comms = MPI.COMM_SELF.Spawn_multiple([sys.executable]*n_inds, args=[[self.path+"/run_optimizee.py", str(gen),str(idx), "&"] for idx in range(n_inds)], maxprocs=1)
        logger.info("Launched individuals")
        
        self.exit_codes = []
        self.exit_codes = sub_comms.allgather(0)
        if self.world_rank == 0:
            logger.info("Exit codes {}".format(self.exit_codes))
    ...
    def prepare_run_file(self):
        ...
        f.write('import pickle\n' +
                ...
                'print("FINISHED ID:{}".format(str(idx)))\n') #EXIT CODES
