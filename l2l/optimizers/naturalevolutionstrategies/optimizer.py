...
NaturalEvolutionStrategiesParameters = namedtuple(
...
class NaturalEvolutionStrategiesOptimizer(Optimizer):
    ...
    def __init__(
        ...
        traj.f_add_parameter(
            "learning_rate_mu", learning_rate_mu, comment="Learning rate mu"
        )
        traj.f_add_parameter(
            "learning_rate_sigma", learning_rate_sigma, comment="Learning rate mu"
        )
        traj.f_add_parameter(
            "mu", parameters.mu, comment="Initial mean of search distribution"
        )
        traj.f_add_parameter(
            "sigma",
            parameters.sigma,
        ...
        current_eval_pop_arr = (
            self.mu + self.sigma * self.current_perturbations
        ).tolist()
    ...
    def post_process(self, traj, fitnesses_results):
        ...
        # **************************************************************************************************************
        self.mu += (
            traj.learning_rate_mu
            * traj.sigma
            * np.dot(fitnesses_to_fit, sorted_perturbations)
        )
        self.sigma *= np.exp(
            traj.learning_rate_sigma
            / 2.0
            * np.dot(fitnesses_to_fit, sorted_perturbations**2 - 1.0)
        )
    ...
    def _compute_utility(self, sorted_fitness):
        ...
        sorted_utilities = np.array(
            [
                max(0.0, np.log((n_individuals / 2) + 1) - np.log(i + 1))
                for i in range(n_individuals)
            ]
        )
        sorted_utilities /= np.sum(sorted_utilities)
        sorted_utilities -= 1.0 / n_individuals
    ...
    def end(self, traj):
        ...
        best_last_indiv_dict = list_to_dict(
            self.best_individual_in_run.tolist(), self.optimizee_individual_dict_spec
        )
