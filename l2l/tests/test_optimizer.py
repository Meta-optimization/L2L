...
class OptimizerTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        jube_params = {}
        self.trajectory_functionGenerator, all_jube_params = (
            self.experiment_functionGenerator.prepare_experiment(
                name="L2L", log_stdout=True, jube_parameter=jube_params, overwrite=True
            )
        )
        self.optimizee_functionGenerator_parameters = namedtuple(
            "OptimizeeParameters", []
        )
        ...
        self.trajectory_activeWait, all_jube_params = (
            self.experiment_activeWait.prepare_experiment(
                name="L2L", log_stdout=True, jube_parameter=jube_params, overwrite=True
            )
        )
        self.optimizee_activeWait_parameters = AWOptimizeeParameters(difficulty=10000.0)
        self.optimizee_activeWait = AWOptimizee(
            self.trajectory_activeWait, self.optimizee_activeWait_parameters
        )
