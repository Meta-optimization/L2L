...
EvolutionStrategiesParameters = namedtuple(
...
class EvolutionStrategiesOptimizer(Optimizer):
    ...
    def __init__(
        ...
        traj.f_add_parameter(
            "learning_rate", parameters.learning_rate, comment="Learning rate"
        )
        traj.f_add_parameter(
            "noise_std", parameters.noise_std, comment="Standard deviation of noise"
        )
        traj.f_add_parameter(
            "mirrored_sampling_enabled",
        ...
        assert (
            noise_std_shape == ()
            or noise_std_shape == self.current_individual_arr.shape
        )
    ...
    def _get_perturbations(self, traj):
        pop_size, noise_std, mirrored_sampling_enabled = (
            traj.pop_size,
            traj.noise_std,
            traj.mirrored_sampling_enabled,
        )
        perturbations = noise_std * self.random_state.randn(
    ...
    def post_process(self, traj, fitnesses_results):
        ...
        if fitness_shaping_enabled:
            ...
            for i in range(n_individuals):
                u = max(0.0, np.log((n_individuals / 2) + 1) - np.log(i + 1))
        ...
        self.current_individual_arr += (
            learning_rate
            * np.sum(
                [f * e for f, e in zip(fitnesses_to_fit, sorted_perturbations)], axis=0
            )
