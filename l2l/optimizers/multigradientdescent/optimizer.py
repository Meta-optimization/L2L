...
class MultiGradientDescentOptimizer(Optimizer):
    ...
    def __init__(
        ...
        traj.f_add_parameter(
            "learning_rate", parameters.learning_rate, comment="Value of learning rate"
        )
        traj.f_add_parameter(
            "exploration_step_size",
        ...
        traj.f_add_parameter(
            "n_inner_params",
            parameters.n_inner_params,
            comment="Number of parameters internally explored per individual",
        )
        traj.f_add_parameter(
            "stop_criterion",
        ...
        traj.f_add_parameter(
            "seed", np.uint32(parameters.seed), comment="Optimizer random seed"
        )
        ...
        if type(parameters) is MultiClassicGDParameters:
            ...
            raise Exception(
                'Class of the provided "parameters" argument is not among the supported types'
            )
        ...
        new_individual_list = [
            list_to_dict(
                ...
                + self.random_state.normal(
                    0.0, parameters.exploration_step_size, self.current_individual.size
                ),
        ...
        if optimizee_bounding_func is not None:
            new_individual_list = [
                self.optimizee_bounding_func(ind) for ind in new_individual_list
            ]
        ...
        self.g = 0
        new_individual_list = self.compress_individual(
            new_individual_list, traj.n_inner_params
        )
    ...
    def post_process(self, traj, fitnesses_results):
        ...
        old_eval_pop_expanded = self.expand_individual(
            old_eval_pop, traj.n_inner_params
        )
        self.eval_pop.clear()
        ...
        if self.g < traj.n_iteration - 1 and traj.stop_criterion > self.current_fitness:
            ...
            self.update_function(
                traj, np.dot(np.linalg.pinv(dx), fitnesses - self.current_fitness)
            )
            ...
            new_individual_list = [
                list_to_dict(
                    self.current_individual
                    + self.random_state.normal(
                        0.0, traj.exploration_step_size, self.current_individual.size
                    ),
                    self.optimizee_individual_dict_spec,
                )
            ...
            new_individual_list_reform = self.compress_individual(
                new_individual_list, traj.n_inner_params
            )
    ...
    def end(self, traj):
        ...
        best_last_indiv_dict = list_to_dict(
            self.current_individual.tolist(), self.optimizee_individual_dict_spec
        )
        ...
        logger.info(
            "The last individual was %s with fitness %s",
            self.current_individual,
    ...
    def rmsprop_update(self, traj, gradient):
        ...
        self.so_moment = traj.momentum_decay * self.so_moment + (
            1 - traj.momentum_decay
        ) * np.multiply(gradient, gradient)
        self.current_individual += np.multiply(
            traj.learning_rate / (np.sqrt(self.so_moment + self.delta)), gradient
        )
    ...
    def adam_update(self, traj, gradient):
        ...
        self.fo_moment = (
            traj.first_order_decay * self.fo_moment
            + (1 - traj.first_order_decay) * gradient
        )
        self.so_moment = traj.second_order_decay * self.so_moment + (
            1 - traj.second_order_decay
        ) * np.multiply(gradient, gradient)
        fo_moment_corrected = self.fo_moment / (
            1 - traj.first_order_decay ** (self.g + 1)
        )
        so_moment_corrected = self.so_moment / (
            1 - traj.second_order_decay ** (self.g + 1)
        )
    ...
    def stochastic_gd_update(self, traj, gradient):
        ...
        gradient += self.random_state.normal(
            0.0, traj.stochastic_deviation, self.current_individual.size
