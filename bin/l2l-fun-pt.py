...
def main():
    ...
    traj, all_jube_params = experiment.prepare_experiment(
        name=name, trajectory_name=name, log_stdout=True
    )
    ...
    optimizee = FunctionGeneratorOptimizee(
        traj, benchmark_function, seed=optimizee_seed
    )
    ...
    temperature_bounds = np.array([[0.8, 0], [0.7, 0], [0.6, 0], [1, 0.1], [0.9, 0.2]])
    ...
    assert ((temperature_bounds.all() <= 1) and (temperature_bounds.all() >= 0)) and (
        temperature_bounds[:, 0].all() > temperature_bounds[:, 1].all()
    ...
    experiment.run_experiment(
        optimizer=optimizer, optimizee=optimizee, optimizer_parameters=parameters
    )
    # End experiment
