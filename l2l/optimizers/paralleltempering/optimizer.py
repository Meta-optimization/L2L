...
ParallelTemperingParameters = namedtuple(
...
AvailableCoolingSchedules = Enum(
    "Schedule",
    "DEFAULT LOGARITHMIC EXPONENTIAL LINEAR_MULTIPLICATIVE QUADRATIC_MULTIPLICATIVE LINEAR_ADDAPTIVE QUADRATIC_ADDAPTIVE EXPONENTIAL_ADDAPTIVE TRIGONOMETRIC_ADDAPTIVE",
)
...
class ParallelTemperingOptimizer(Optimizer):
    ...
    def __init__(
        ...
        traj.f_add_parameter(
            "noisy_step", parameters.noisy_step, comment="Size of the random step"
        )
        traj.f_add_parameter(
            "n_iteration",
        ...
        new_individual_list = [
            list_to_dict(
                ind_as_list
                + np.random.normal(0.0, parameters.noisy_step, ind_as_list.size)
                * traj.noisy_step,
        ...
        schedule_known = (
            True  # start off as True - if any schdule is unknown gets False
        )
        for i in range(np.size(self.cooling_schedules)):
            schedule_known = (
                schedule_known
                and self.cooling_schedules[i] in AvailableCoolingSchedules
            )
    ...
    def cooling(
        ...
        # Additive monotonic cooling B. T. Luke (2005)
            ...
            return temperature_end + (T0 - temperature) * (
                (steps_total - k) / steps_total
            )
        elif cooling_schedule == AvailableCoolingSchedules.QUADRATIC_ADDAPTIVE:
            return temperature_end + (T0 - temperature) * np.square(
                (steps_total - k) / steps_total
            )
            ...
            return temperature_end + (T0 - temperature) * (
                ...
                / (
                    1
                    + np.exp(
                        (2 * np.log(T0 - temperature_end) / steps_total)
                        * (k - steps_total / 2)
                    )
                )
            )
            ...
            return (
                temperature_end
                + (T0 - temperature_end) * (1 + np.cos(k * 3.1415 / steps_total)) / 2
            )
    ...
    def post_process(self, traj, fitnesses_results):
        ...
        for i in range(0, traj.n_parallel_runs):
            self.T_all[self.parallel_indices[i]] = self.cooling(
                temperature[self.parallel_indices[i]],
                cooling_schedules[self.parallel_indices[i]],
                decay_parameters[self.parallel_indices[i]],
                temperature_bounds[self.parallel_indices[i], :],
                n_iteration,
            )
        ...
        for i, (run_index, fitness) in enumerate(fitnesses_results):
            ...
            new_individual = list_to_dict(
                current_individual
                + np.random.randn(current_individual.size) * noisy_step * self.T,
                self.optimizee_individual_dict_spec,
            )
        ...
        for i in range(0, traj.n_parallel_runs):
            ...
            if (
                self.metropolis_hasting(
                    self.current_fitness_value_list[i],
                    self.current_fitness_value_list[random_choice],
                    self.T_all[i],
                    self.T_all[random_choice],
                )
                > random_variable
            ):
        ...
        logger.debug(
            "Current best fitness within population is %.2f",
    ...
    def end(self, traj):
        ...
        best_last_indiv_dict = list_to_dict(
            best_last_indiv.tolist(), self.optimizee_individual_dict_spec
        )
        ...
        logger.info(
            "The best last individual was %s with fitness %s",
            best_last_indiv,
            best_last_fitness,
        )
