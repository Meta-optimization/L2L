...
class SimulatedAnnealingOptimizer(Optimizer):
    ...
    def __init__(
        ...
        self.T = 1.0  # Initialize temperature
        ...
        new_individual_list = [
            list_to_dict(
                ind_as_list
                + self.random_state.normal(0.0, parameters.noisy_step, ind_as_list.size)
                * traj.noisy_step
                * self.T,
    ...
    def post_process(self, traj, fitnesses_results):
        ...
        temperature_end = 0
        self.T = self.cooling(
            temperature, self.cooling_schedule, temp_decay, temperature_end, n_iteration
        )
        ...
        for i, (run_index, fitness) in enumerate(fitnesses_results):
            ...
            new_individual = list_to_dict(
                current_individual
                + self.random_state.randn(current_individual.size)
                * noisy_step
                * self.T,
                self.optimizee_individual_dict_spec,
            )
